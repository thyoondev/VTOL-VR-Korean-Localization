...
## 2차 패스 정책 (인게임 피드백 반영, 2026-08-03)

### 폭이 좁은 라벨 = 영문 유지
HUD/MFD 등 고정 폭 UI에서 한글이 잘리는 문제(예: BRAKE→"브레이크"가 "브레이"로 잘림) 해결책:
- **고정폭 라벨 판별**: 키가 `Hud*`, `Label*`, `txt_*`이거나, en이 전부 대문자/대괄호 포함(`[ BRAKE LOCK ]`), Description에 "HUD"/"sub-label" 포함 → 고정폭으로 간주
- 고정폭 라벨에서 ko 표시 폭(한글=2, 영문·숫자=1)이 en 폭보다 크면 **영문 원문으로 되돌린다** (예: BRAKE, BINGO LEVEL, TERMINAL MODE)
- VR Interactable(잡기/클릭 대상 이름)은 자유 폭 툴팁이므로 한국어 유지
- 버튼/캔버스 텍스트는 ko 폭이 en 폭의 1.5배를 넘지 않게 다듬는다

### 쉬운 표현 원칙 (비전문가 대상)
설명문에서 전문용어를 일상어로 풀어쓴다. 라벨은 짧게, 설명은 쉽게:
- 종말 유도 → (설명문) "명중 직전 단계의 비행 방식"
- 반능동 레이더 유도 → "발사 후에도 기체 레이더로 계속 조준을 유지해야 하는 방식"
- 발사 후 망각 → "발사 후 알아서 유도되는(발사 후 망각)"
- 기만체 → 기만 장비, 채프 → (첫 등장 시) "레이더 교란용 채프", 플레어 → "적외선 교란용 플레어"
- 받음각 → (설명문) "기수 방향과 실제 진행 방향 사이의 각도(받음각)"
- 조준 고정(록온), 포착 → "표적을 잡다" 계열의 자연스러운 표현 허용
- 원칙: 라벨에서 못 푸는 용어는 설명문·튜토리얼에서 한 번 풀어준다

### 줄 바꿈 원칙
- 여러 줄 셀: 줄 수는 en과 같게(세로 텍스트 제외), **줄은 구절 경계에서만 끊는다** (조사·명사 중간 금지)
- 각 줄 길이를 비슷하게 배분해 가독성 확보
